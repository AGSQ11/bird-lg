...
def inject_commands():
    commands = [
            ...
            ("detail_import", "show route protocol ..."),
            ("detail_best", "show route primary protocol ..."),
            ("detail_filtered", "show route filtered protocol ..."),
            ("detail_export", "show route export ..."),
            ("adv", "show route ..."),
...
@app.route("/detail_best/<hosts>/<proto>")
def detail_best(hosts, proto):
    name = get_query()

    if not name:
        abort(400)

    set_session("detail_best", hosts, proto, name)
    command = "show route primary protocol %s" % name
    ...
    for host in hosts.split("+"):
        ...
        if len(res) <= 1:
            ...
            continue
	topr = "\n".join(res);
        detail_best[host] = {"status": command, "description": add_links(topr)}
         

    return render_template('detail.html', detail=detail_best, command=command, errors=errors)
	
@app.route("/detail_filtered/<hosts>/<proto>")
def detail_filtered(hosts, proto):
    name = get_query()

    if not name:
        abort(400)

    set_session("detail_filtered", hosts, proto, name)
    command = "show route filtered protocol %s" % name
    ...
    for host in hosts.split("+"):
        ...
        if len(res) <= 1:
            ...
            continue
	topr = "\n".join(res);
        detail_filtered[host] = {"status": command, "description": add_links(topr)}
         

    return render_template('detail.html', detail=detail_filtered, command=command, errors=errors)
	

@app.route("/detail_import/<hosts>/<proto>")
def detail_import(hosts, proto):
    name = get_query()

    if not name:
        abort(400)

    set_session("detail_import", hosts, proto, name)
    command = "show route all protocol %s" % name
    ...
    for host in hosts.split("+"):
        ...
        if len(res) <= 1:
            ...
            continue
	topr = "\n".join(res);
        detail_import[host] = {"status": command, "description": add_links(topr)}

    return render_template('detail.html', detail=detail_import, command=command, errors=errors)

@app.route("/detail_export/<hosts>/<proto>")
def detail_export(hosts, proto):
    name = get_query()

    if not name:
        abort(400)

    set_session("detail_export", hosts, proto, name)
    command = "show route all export %s" % name
	...
	topr = "\n".join(res);
        detail_export[host] = {"status": command, "description": add_links(topr)}

    return render_template('detail.html', detail=detail_export, command=command, errors=errors)
